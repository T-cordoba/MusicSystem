...
from sistema_música.model.sis_música import Song, User, SysMusic,AudioPlayer
...
class SysMusicGUI:
    ...
    def __init__(self, sys_music_fix: SysMusic):
        self.sys_music_fix = sys_music_fix
    ...
    def start_menu_gui(self):
        ...
        window = sg.Window('Sistema de música', layout)
        while True:
    ...
    def user_info_gui(self):
        while True:
            layout = [
                [sg.Text('Ingrese su nombre:'), sg.InputText(key='-NAME-')],
                [sg.Text('Ingrese su email:'), sg.InputText(key='-EMAIL-')],
                [sg.Text('Ingrese su edad:'), sg.InputText(key='-AGE-')],
                [sg.Text('Ingrese su ocupación:'), sg.InputText(key='-OCCUPATION-')],
                [sg.Text('Ingrese su país:'), sg.InputText(key='-COUNTRY-')],
                [sg.Button('Ok')]
            ]
            window = sg.Window('Información del usuario', layout)
            while True:
                event, values = window.read()
                if event == sg.WINDOW_CLOSED:
                    ...
                    try:
                        ...
                        self.sys_music_fix.user = User(name, email, int(age), occupation, country)
                        window.close()
                        return
    ...
    def add_song_gui(self, file_path):
        layout = [
            ...
            [sg.Text('Genro:'), sg.InputText(key='-GENRE-')],
    ...
    def create_playlist_gui(self):
        ...
        while True:
            ...
            if event == 'OK':
                ...
                self.sys_music_fix.user.create_playlist(playlist_name)
                break
        window.close()

    def view_playlist_gui(self, playlist_name):
        playlist = self.sys_music_fix.user.playlists[playlist_name]
        ...
        layout = [
            ...
            [sg.Button('Añadir canción'), sg.Button('Eliminar canción')],
            [sg.Text('Canciones disponibles:')],
            [sg.Listbox(values=[f"{song.title} - {song.artist} - {song.genre}" for song in self.sys_music_fix.songs],
        ...
        while True:
            ...
            if event == sg.WINDOW_CLOSED or event == 'OK':
                ...
                if values['-SONG LIST AVAILABLE-']:
                    ...
                    for song in self.sys_music_fix.songs:
                        if [song.title, song.artist, song.genre] == selected_song_details:
                            self.sys_music_fix.user.add_song_to_playlist(song, playlist_name)
                            playlist = self.sys_music_fix.user.playlists[playlist_name]
                ...
                if values['-SONG LIST-']:
                    ...
                    for song in playlist.songs:
                        if [song.title, song.artist, song.genre] == selected_song_details:
                            self.sys_music_fix.user.remove_song_from_playlist(song, playlist_name)
                            playlist = self.sys_music_fix.user.playlists[playlist_name]
                            window['-SONG LIST-'].update(
                                [f"{song.title} - {song.artist} - {song.genre}" for song in playlist.songs])
                            break
    ...
    def music_player_gui(self):
        layout = [
            [sg.Button('Información del usuario')],
            [sg.Text(f'Hola, {self.sys_music_fix.user.name}')],
            [sg.Text('')],
            [sg.Input(key='-FILE-', visible=False, enable_events=True),
             sg.FileBrowse(file_types=(("MP3 Files", "*.mp3"),))],
            [sg.Text('Canciones disponibles:'), sg.Text('                                                     '),
             sg.Text('Canciones en la cola:')],
            [sg.Listbox(values=[], size=(50, 20), key='-SONG LIST-', bind_return_key=True),
             sg.Listbox(values=[], size=(50, 20), key='-QUEUE-', bind_return_key=True)],
            [sg.Button('Editar'), sg.Button('Eliminar')],
            [sg.Text('')],
        ...
        window = sg.Window('Sistema de música', layout, finalize=True)
        window['-SONG LIST-'].update(
            [f"{song.title} - {song.artist} - {song.genre}" for song in self.sys_music_fix.songs])
        paused = False
        while True:
            event, values = window.read()
            if event == sg.WINDOW_CLOSED:
            ...
            if event == '-FILE-':
                ...
                if song_details is not None:
                    song = Song(song_details['-TITLE-'], song_details['-ARTIST-'], song_details['-GENRE-'],
                                song_file_path)
                    try:
                        self.sys_music_fix.add_song(song)
                ...
                if values['-SONG LIST-']:
                    ...
                    for song in self.sys_music_fix.songs:
                        if [song.title, song.artist, song.genre] == selected_song_details:
                            self.sys_music_fix.audio_player.play(song)
                            break
                else:
                    sg.popup('Por favor, selecciona una canción antes de presionar reproducir')
            elif event == 'Pausar':
                if self.sys_music_fix.audio_player.is_playing:
                    if paused:
                        self.sys_music_fix.audio_player.unpause()
                        window['Pausar'].update('Pausar')
                        paused = False
                    else:
                        self.sys_music_fix.audio_player.pause()
                ...
                if self.sys_music_fix.audio_player.is_playing:
                    window['Pausar'].update('Pausar')
                    paused = False
                    self.sys_music_fix.audio_player.stop()
                else:
                ...
                if values['-SONG LIST-']:
                    ...
                    for song in self.sys_music_fix.songs:
                        if [song.title, song.artist, song.genre] == selected_song_details:
                            self.sys_music_fix.remove_song(song)
                            window['-SONG LIST-'].update(
                                [f"{song.title} - {song.artist} - {song.genre}" for song in self.sys_music_fix.songs])
                            break
                else:
                ...
                if values['-PLAYLIST LIST-']:
                    selected_playlist = values['-PLAYLIST LIST-'][0]
                    playlist = self.sys_music_fix.user.playlists[selected_playlist]
                    self.sys_music_fix.audio_player.play_playlist(playlist)
                    window['-QUEUE-'].update(
                        [f"{song.title} - {song.artist} - {song.genre}" for song in
                         self.sys_music_fix.audio_player.queue])
                else:
                    sg.popup('Por favor, selecciona una playlist antes de presionar reproducir playlist')
            elif event == 'Siguiente':
                self.sys_music_fix.audio_player.next()
                window['-QUEUE-'].update(
                    [f"{song.title} - {song.artist} - {song.genre}" for song in
                     self.sys_music_fix.audio_player.queue])
                ...
                if values['-SONG LIST-']:
                    ...
                    for song in self.sys_music_fix.songs:
                        if [song.title, song.artist, song.genre] == selected_song_details:
                            self.sys_music_fix.audio_player.add_to_queue(song)
                            window['-QUEUE-'].update(
                                [f"{song.title} - {song.artist} - {song.genre}" for song in
                                 self.sys_music_fix.audio_player.queue])
                            break
